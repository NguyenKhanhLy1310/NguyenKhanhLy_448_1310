class PlayFairCipher:
    ...
    def playfair_encrypt(self, plain_text, matrix):
        # 1. Chuẩn hóa: Viết hoa, chuyển J thành I
        plain_text = plain_text.upper().replace("J", "I")
        
        # 2. QUAN TRỌNG: Chỉ giữ lại các chữ cái tiếng Anh hợp lệ (loại bỏ khoảng trắng, dấu, số)
        ...
        for i in range(0, len(plain_text), 2):
            ...
            # Xử lý trường hợp 2 ký tự trùng nhau trong cùng một cặp (Ví dụ: HELLO -> HE LX LO)
            if pair[0] == pair[1]:
                pair = pair[0] + "X"
                # Giảm i đi 1 để ký tự thứ 2 sẽ được xét tiếp ở cặp sau
                i -= 1 
